...
#Using model b for fitting the model
coeff_fit <- summary(fit_b)$coefficient
ci_95_beta0 <- coeff_fit[1,1] + c(-1,1) * qt(.975, df = fit_b$df) * coeff_fit[1,2] 
ci_95_beta1 <- coeff_fit[2,1] + c(-1,1) * qt(.975, df = fit_b$df) * coeff_fit[2,2] 

```  
#Executive Summary
According to the selected linear regression model __automatic__ models are better for `mpg`. Actually __automatic__ models have an estimated mpg of `r  round(coeff_fit[1,1],3)` (miles per gallon)  with a 95% confidence interval included in [`r round(ci_95_beta0[1],3)` - `r round(ci_95_beta0[2],3)`] while __manual__ models have an increased estimated mpg of `r  round(coeff_fit[2,1],3)` (miles per gallon).

#Data Analysis
The `mtcars` dataset include `r dim(mtcars)[1]` observations of `r dim(mtcars)[2]` features. Each onservations comprises some information for a specific automobiles (1973 - 1974 model). The focus of this investigation is two specific features __mpg__ (miles per gallon) and __am__ (type of transmission - 0: automatic, 1: manual). 

The available datase contains `r table(mtcars$am)[[1]][1]` automatic car models and `r table(mtcars$am)[[2]][1]` manual car models. The observed `mpg` by type of transmission `am` can be seen in the provided hystogram (see __Appendix, Figure 1__). 

From the hystogram we can see that `am` (predictor) is visibly related to `mpg` (outcome) as we can see from the sample mean of each group. The "automatic" group has a lower sample mean (`r round(mpg_avg_0,3)` Miles/ Gallon) than the "manual" group (`r round(mpg_avg_1,3)` Miles/ Gallon).

#Regression Model

##A Simple Model
The simpler  linear model, we can build, uses `mpg` (as outcome) with `am` as predictor (__`mpg ~ am`__). The coefficients of the fitted model are     
```{r echo=FALSE}
summary(fit_a)$coefficient
```  
From the Residual vs Fitted plot (see __Appendix, Figure 2__) we can see that the model is just able to predict two possible values for the estimated `mpg` based on the predictor - __17.147 mpg for automatic models__ and __24.392 mpg for manual models__ with an estimated residuals standard error of __`r round(summary(fit_a)$sigma,3)` mpg__. While the Q-Q plot (see __Appendix, Figure 2__) confirms the normality of the errors (assumption). A more detailed interpration of the simple model can be found in __Appendix, Simple Model Interpretation__.

##Extending the Model adding new features  
The simple model is quite limited. Looking at the available features, other possible features could be used to identify a "better" model. A possible feature that may be valuable investigating is `hp` (__gross horse power__ in hp). The relationship between `mpg` vs. `hp` by `am` can be seen in __Appendix, Figure 3__.   
...
anova(fit_a, fit_b)
```  
Model 2 (__`mpg ~ am + hp`__) seems to provide a better "description" of the response and it will be used to answer the original questions. See the Residual vs Fitted plot and Q-Q plot (see __Appendix, Figure 4__). 
...
* __automatic__ models (reference group - `am = 0`) have an estimated mpg of `r  round(coeff_fit[1,1],3)` (miles per gallon) with a standard error of `r  round(coeff_fit[1,2],3)` (miles per gallon).

* __manual__ models (`am = 1`) have an increased estimated mpg of `r  round(coeff_fit[2,1],3)` (miles per gallon) (over the reference group) with a standard error of `r  round(coeff_fit[2,2],3)` (miles per gallon). The P-value of `r  round(coeff_fit[2,4],3)` is statistically significant, reject the __null hypothersis__ (having an increase/ decrease over the reference group null).
...
#Appendix  
##Figure1: Hystogram of observed `mpg` by `am` (type of transmission)  
...
##Figure2: Residual Plots for (_`mpg ~ am`_) model
...
##Figure3: (_`mpg vs. hp`_) by _`am`_ (transmission type) plots 
...
##Figure4: Residual Plots for (_`mpg ~ am + hp`_) model  
